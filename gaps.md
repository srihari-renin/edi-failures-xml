...
- **Follow-up 2026-08-28:** the penny adjustment applied during troubleshooting did **not** fix the posting error, and it *did* cause a second, separate defect — it moved the sales line price 3 cents off the PO price, which the invoice then inherited (see [810 Case 01](.claude/skills/edi-xml-failure-corrections/cases/home-depot-ca-mdo-810.md#case-01--810--price-mismatch--2026-08-28)). So the technique cost more than it returned here. It should not be reached for again without diffing the resulting invoice against its PO; it is now flagged as a cross-cutting rule in `SKILL.md`'s General notes.
